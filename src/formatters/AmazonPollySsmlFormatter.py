    ...
    def get_text_modifier_object(self, ast):
        ...
        for child in ast['children']:
            if child['name'] in ['plainText', 'plainTextSpecialChars', 'plainTextEmphasis', 'plainTextPhone', 'plainTextModifier']:
                ...
                text_modifier_object.tag(ssml_tag, {'interpret-as': key, 'format': value or 'default'})
        return text_modifier_object

    def format_from_ast(self, ast, lines=[]):
        # This method should follow a similar approach to handling AST nodes as seen in previous formatters
...
formatter = AmazonPollySsmlFormatter(options)
